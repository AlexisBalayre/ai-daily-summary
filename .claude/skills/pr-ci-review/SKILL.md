...
allowed-tools: Bash(git *), Bash(gh *), Bash(uv *), Bash(npm *), Read, Edit, Write, Grep, Glob, Agent
...
- **Leave linters alone:** do not raise anything ruff (lint + format, `LINT_CMD` / `FORMAT_FIX_CMD` in `.claude/project.env`) or the frontend's ESLint + `tsc` already catches; that is noise, not a finding.
...
- **local**: the working-tree diff against the merge-base with the trunk (`master`, `GIT_TRUNK` in `.claude/project.env`): committed + staged + unstaged.
...
- [ ] Gate the run: config-restore awareness under CI, freshness for a local pr, pre-flight ruff + pytest (+ `npm run lint`) suite for local source.
