...
The hook dispatches on `//` line comments, `/* */` block comments (and `*` continuation lines), and `# ` hash comments (hash + space, so shebangs and `#include`-style directives are not counted). Judge doc comments in the same files too (docstrings, JSDoc).

Skip entirely: generated files (any path matching `GENERATED_PATHS_REGEX` in `.claude/project.env`), Alembic migrations under `ai_daily/db/migrations/versions/`, and anything under `node_modules`, `dist`, `build`, `.venv`, `__pycache__`, `ai_daily/static/`, `archive`, or another build-output or vendored-dependency directory.
...
`# noqa` / `# noqa: <codes>`, `# ruff: noqa`, `# type: ignore[...]`, `# pyright: ignore`, `# mypy: ...`, `# pylint: disable=...`, `# pragma: no cover`, `# fmt: off`/`# fmt: on`, `# isort: skip`, `@ts-expect-error <reason>`, `@ts-ignore`, `eslint-disable*` and any other linter's ignore/disable directive, `// prettier-ignore`, shebangs, encoding (`# -*- coding: utf-8 -*-`) and license/SPDX headers.

**Tier 2, tooling-coupled keeps.** Kept because deleting them loses an external fact not in the code:

1. Comments quoting a ticket/issue ID plus context (`# GH-142: Gemini streams receipts out of order; buffer defensively`). The ticket is an external fact not in the code.
2. The **existence** of a doc comment summary line the project's conventions make mandatory (e.g. a public-symbol docstring summary). Deleting it trips the missing-doc check and prune-only means nothing refills it. Keep the line. It is still subject to `doc-rambling` trimming, but do not apply `doc-name-restate` to it: a name-restating summary on a required symbol stays, because the existence requirement outranks the restatement.
...
- **Tests get mechanical categories only.** In test files (`test_*.py`, `conftest.py`, `*.test.*`, `*.spec.*`, `__mocks__/`, `tests/`), act only on `divider`, `journal`, `commented-code`, and `file-banner`. Do not apply the content-judgment rules there (`restate-what`, `weak-why`, `stale-todo`, `schema-restate`, `seam-duplicate`, or any `doc-*`).
...
trimmed  ai_daily/db/models.py:10     [doc-rambling]   (kept summary line, dropped 4 body lines)
```
