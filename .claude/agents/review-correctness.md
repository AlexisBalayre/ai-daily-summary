...
The project's deterministic suite (`uv run ruff check`, `uv run ruff format`, `uv run pytest`, i.e. `LINT_CMD` and `TEST_CMD` in `.claude/project.env`, plus CI's frontend `tsc -b` and `eslint`) catches everything mechanical: syntax, lint, types, imports, formatting, and the regressions an existing test already covers. Stay out of its territory and never report anything it would flag. Your mandate is the layer above, the defects only judgment finds: does this code actually do the right thing?
