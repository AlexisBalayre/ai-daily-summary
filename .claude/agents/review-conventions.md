...
- `docs/conventions/*`: the primary, authoritative rules. The universal `core.md` applies to every source file, `testing.md` to tests, and each other area doc (`api.md`, `etl.md`, `database.md`, `outputs.md`, `orchestrator.md`, `frontend.md`) to the paths its `.claude/rules/*-conventions.md` loader names; each auto-loads when you read a file in that area. Weight these first. A section still marked `TODO(adapt)` states no rule.
...
- `AGENTS.md` (imported by `CLAUDE.md`): a smaller complementary set of guidelines (module map, git workflow, key commands). Apply it too.
...
Quote means read. Open the rule, ADR, precedent file, or callee docstring you cite and take its exact text from this tree, with file and line, before it appears in a finding. A premise recalled from memory (a sibling that "always does X", a docstring that "does not cover this", an ADR attributed by number, a rule `AGENTS.md` does not actually state) is where this reviewer's refuted findings come from; if you have not read it, you cannot cite it.
