...
This config is built around one habit: **never work on the trunk (`master` here), one git
...
- **No accidental commits to `master`.** Plain `git commit` from a worktree lands on that
...
```
scripts/worktree-create.sh my-feature   # .worktrees/my-feature on branch feat/my-feature
  └─ cd .worktrees/my-feature
       ...
       ├─ commit (pre-commit runs TEST_CMD; lands on feat/ branch, never master)
...
| Worktree creation | `scripts/worktree-create.sh` | `scripts/worktree-create.sh <name>` always makes `.worktrees/<name>` on `feat/<name>` and runs `INSTALL_CMD` (`uv sync`). |
...
The trunk and branch prefix are `GIT_TRUNK` (`master` here) and `WORKTREE_BRANCH_PREFIX`
(`feat` here) in `.claude/project.env`. The worktree scripts and `.claude/hooks/git-safety.sh`
both read them from there, so the helper and the guard stay in agreement automatically. The loop
above assumes an `origin` remote.
